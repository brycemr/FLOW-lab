...
Objective: minimize -E/E0
Design Variables: Turbine Positions (x,y)
Parameters: Wind Rose
Constraints: Wind Farm Area (For circular farm, x1^2 + y1^2 <= radius for all x,y pairs, for rectangular can be applied as design constraint upper and lower bounds) 

Questions:
Do I form a g(x) constraint equation for each turbine pair? Or can I summarize that in a single equation/constraint? Should these
be applied as design constraints on the design variable?
"""
...
test = [[0,0],[0,0]]
print(WindfarmOutputRatio(test,0,10))
